...
<div class="py-3 mb-4 shadow-sm bg-warning border-top">
    <div class="container">
        ...
        <a href="{{url('category_product/'.$product->category->slug)}}" class="text-success">Category /</a>
        <a href="{{url('category_product/'.$product->category->slug)}}" class="text-success">{{ $product->category->name}}&nbsp;&nbsp;/
        </a>
        <a href="{{url('category/'.$product->category->slug.'/'.$product->slug)}}" class="text-success">{{ $product->name}}</a>  
        </h6>
    </div>
</div>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-8" style="position:relative;">
                        ...
                        <label class="mb-3">Original Price : <s><b>&#2547;</b>&nbsp;{{$product->original_price}}</s></label>
                    &nbsp;&nbsp;&nbsp;<label class="font-weight-bold">Selling Price : <b>&#2547;</b>&nbsp;{{$product->selling_price}}</label>
                    <p class="mt-3 text-dark">{!! $product->small_description	!!}</p>
                    <hr>

                    @if ($product->qty > 0)

                    <label class="badge badge-success mr-2">In stock</label> (<small class="text-danger m-2 font-weight-bold">{{ $product->qty}} available</small>)
                    @else
